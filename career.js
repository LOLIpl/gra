...
function renderFormation() {
    ...
    state.lineup.forEach((p,i)=>{const sl=df.positions[i]||{top:50,left:50};const n=document.createElement("div");n.className="player-slot";n.draggable=true;n.dataset.playerId=String(p.id);n.dataset.zone="lineup";n.style.top=`${sl.top}%`;n.style.left=`${sl.left}%`;n.style.transform="translate(-50%,-50%)";n.onclick=()=>showPlayerModal(p);n.addEventListener("mouseenter",(e)=>showPlayerTooltip(p,e));n.addEventListener("mousemove",(e)=>{const el=document.getElementById("playerTooltip");if(el&&el.style.display==="block")positionTooltip(el,e);});n.addEventListener("mouseleave",hidePlayerTooltip);const bc=ratingBorderColor(p.rating);const rc=ratingClass(p.rating);const rbc=ratingBorderColor(p.rating);const fitPct=p.fitness||96;const fitColor=fitPct>=85?'#10b981':fitPct>=65?'#f59e0b':'#ef4444';const photoInner = p.photo_url
            ? `<img src="${escapeHtml(p.photo_url)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><span style="display:none;width:100%;height:100%;align-items:center;justify-content:center;font-size:.95rem;font-weight:800;">${i+1}</span>`
            : `<span style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:.95rem;font-weight:800;">${i+1}</span>`;
        n.innerHTML=`<div class="shirt" style="background:${teamColor};border-color:${bc};box-shadow:0 0 6px ${bc}44,0 3px 8px rgba(0,0,0,.4);overflow:hidden;"><div class="rating-badge" style="background:${rbc};">${p.rating}</div>${photoInner}<div class="fit-indicator" style="position:absolute;bottom:-3px;left:50%;transform:translateX(-50%);width:40px;height:4px;background:rgba(0,0,0,.5);border-radius:2px;overflow:hidden;"><div style="width:${fitPct}%;height:100%;background:${fitColor};border-radius:2px;"></div></div></div><div class="name">${escapeHtml(p.name.split(' ').pop())}</div>`;n.addEventListener("dragstart",onPlayerDragStart);n.addEventListener("dragover",onPlayerDragOver);n.addEventListener("drop",onPitchPlayerDrop);n.addEventListener("dragend",onPlayerDragEnd);d.appendChild(n);});
}
...
function showMatchModeModal(fx){
    ...
    const logo=(url,color,name)=>`<div style="width:52px;height:52px;border-radius:50%;background:${color};margin:0 auto .3rem;${url?`background-image:url('${url}');background-size:76%;background-repeat:no-repeat;background-position:center;`:'display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:800;'}"></div>`;
    const teams=`<div style="display:flex;align-items:center;justify-content:center;gap:1.25rem;margin-bottom:1.5rem;">
        <div style="text-align:center;">${logo(hLogo,hColor,h?.name)}<div style="font-size:.78rem;font-weight:700;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(h?.name||'')}</div></div>
        <div style="font-size:1.4rem;font-weight:900;color:var(--muted);">vs</div>
        <div style="text-align:center;">${logo(aLogo,aColor,a?.name)}<div style="font-size:.78rem;font-weight:700;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(a?.name||'')}</div></div>
    </div>`;
    const c=document.getElementById('modalContent');
    c.innerHTML=`<div style="text-align:center;">
        <h2 style="margin:.25rem 0 .5rem;">Wybierz tryb meczu</h2>${badge}${teams}
...
function snapToMatchDay(d, isEuropean){
    // Liga: piątek(5), sobota(6), niedziela(0), poniedziałek(1)
    // Puchary europejskie: wtorek(2), środa(3), czwartek(4)
    const leagueDays = [5, 6, 0, 1];
    const euroDays   = [2, 3, 4];
    const allowed = isEuropean ? euroDays : leagueDays;
    const day = d.getDay();
    if (!allowed.includes(day)) {
        // Znajdź najbliższy dozwolony dzień do przodu
        let offset = 1;
...
// === EKSPORT DO WINDOW (wymagany gdy career.js ładowany jako type="module") ===
window.initCareer           = initCareer;
window.closeModal           = closeModal;
window.simulateDay          = simulateDay;
window.skipToDate           = skipToDate;
window.acceptSaleOffer      = acceptSaleOffer;
window.listPlayerForSale    = listPlayerForSale;
window.rejectSaleOffer      = rejectSaleOffer;
window.removePlayerFromSale = removePlayerFromSale;
window.renewContract        = renewContract;
window.sellPlayer           = sellPlayer;
window.submitOffer          = submitOffer;
window.toggleNegotiation    = toggleNegotiation;
window.applySeasonOutcome   = applySeasonOutcome;
window.showMatchModeModal   = showMatchModeModal;
window.runQuickSim          = runQuickSim;
window.runLiveSim           = runLiveSim;
window.startSimulation      = startSimulation;
window.showSeasonEndModal   = showSeasonEndModal;
window.showMatchResult      = showMatchResult;
window.confirmCoach         = confirmCoach;
